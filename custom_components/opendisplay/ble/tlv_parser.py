"""TLV configuration parser for OpenDisplay BLE firmware.

Parses the complete device configuration from 0x0040 (Read Config) response.
Based on structs.h from OpenDisplay_BLE firmware.
"""
import struct
import zlib
...
class DisplayConfig:
    ...
    open_display_tagtype: int  # Legacy OpenDisplay tag type
...
def parse_tlv_config(data: bytes) -> GlobalConfig:
    ...
    # Parse packets (OpenDisplay format: [packet_number:1][packet_id:1][fixed_data])
    offset = tlv_start
...
def extract_display_capabilities(config: GlobalConfig) -> DeviceCapabilities:
    ...
    Used by OpenDisplayProtocol.interrogate_device() to return only what HA needs.
...
def config_to_dict(config: GlobalConfig) -> dict[str, Any]:
    ...
    Converts the complete OpenDisplay configuration structure to a nested dictionary
