...
def get_hub_from_hass(hass: HomeAssistant):
    ...
    filtering out BLE entries which are OpenDisplayBLEData instances.

    Args:
        hass: Home Assistant instance

    Returns:
        Hub: The OEPL AP Hub instance
...
def is_ble_entry(entry_data) -> bool:
    ...
    """
    return isinstance(entry_data, OpenDisplayBLERuntimeData)
...
def rgb_to_rgb332(rgb: tuple[int, int, int]) -> str:
    ...
    to the 8-bit RGB332 format used by OpenDisplay for LED patterns.
